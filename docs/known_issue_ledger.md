...
| Executable spine still lost latest airline/retail frames | metrics/trajectories (63)/(64) show airline `0/45` with 1028 generic responds, 697 repeat-loop failures, retail-auth leakage into airline name+ZIP lookup, “want to fly from…” not recognized as booking, staged confirmations swallowed by repeat-loop, and retail exact-or-skip selecting weaker no-backlight keyboard fallback | Deterministic belief state existed but confirmations/closures still flowed through retry-style gates, booking intent verbs were too narrow, retail auth stage was reachable from airline, `respond` pseudo-schema could reject terminal closure, and exact-or-skip allowed fallback rules | Make the executable gradient spine authoritative for forced actions: domain-guard retail auth, bind “fly from X to Y” as booking, fetch profile before search, let forced staged confirms/commits/closures bypass repeat-loop, synthesize required-free response schemas, and disable fallback rules when user says to skip that item | `test_v3_latest_airline_fly_route_fetches_profile_before_search`, `test_v3_latest_airline_task_frame_never_uses_retail_auth`, `test_v3_latest_forced_respond_bypasses_repeat_loop_gate`, `test_v3_latest_retail_actual_exact_or_skip_phrase_blocks_no_backlight_fallback`, `test_v3_latest_respond_schema_has_no_required_content_arg` | Fixed, verified |
| Gate rejection only critiqued the model instead of executing the spine | metrics/trajectories (66) still show airline `0/5`: after a profile read, generic greeting proposals were rejected with `gradient_match`/`repeat_loop`, but the deterministic reservation scan was not executed; forced confirmation asks then failed `ask_user_repeats_same_question`, and downgrade tasks treated “refund to original payment method” as cancel intent | Forced-next actions were logged in per-step diagnostics but not persisted as controller state, forced ASK responses still hit generic repeated-ask state validity, and cancel intent extraction conflated refund method with cancellation | Add a single-slot executable forced-action queue consumed before the proposer, allow forced confirmation/closure responses through state validity, preserve required-arg checks while skipping unverifiable NL preconditions for staged commits, ask for missing airline identity before search, and separate refund-method wording from cancel intent | `test_v3_latest_profile_generic_failure_queues_reservation_scan`, `test_v3_latest_forced_confirmation_bypasses_repeated_ask_state_gate`, `test_v3_latest_airline_booking_without_identity_asks_before_search`, `test_v3_latest_airline_refund_method_does_not_mean_cancel_when_downgrading` | Fixed, verified |
