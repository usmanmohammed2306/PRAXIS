...
SYSTEM_PROMPT = (
    ...
    "verifier finds your action cites unverified facts, ungrounded IDs, "
    "conflicts with task state, is semantically incomplete, or violates a "
    "post-condition you declared, it will REJECT your action and "
    ...
    "- Placeholder values like 'user@example.com' / 'customer@example.com'\n"
    "  are NEVER valid tool arguments — the verifier will block them.\n"
    "- If you need a credential the user has not provided, ask for it with\n"
    "  the ASK_USER pattern shown below.\n"
    "- Treat constraints as hard filters. Preferences only rank candidates\n"
    "  after all hard constraints and required slots are satisfied.\n"
    "- READ actions are for gathering information and may be used before the\n"
    "  task is complete. WRITE/IRREVERSIBLE/FINAL actions are commitments and\n"
    "  must wait until every open slot and obligation is closed.\n"
    "- If STATE already shows the user's intent, route/date/options, or other\n"
    "  requested facts, do not ask for them again; choose the next grounded\n"
    "  READ or a complete verified commit action.\n"
    "\n"
    "ASK_USER / FINAL format — use EXACTLY this shape:\n"
    "  {\"name\": \"respond\", \"args\": {}, \"declared_class\": \"ASK_USER\",\n"
    "   \"user_text\": \"<your question here>\"}\n"
    "  NEVER put 'respond' inside an args field. NEVER use a domain tool\n"
    "  name (e.g. find_user_id_by_email) with declared_class ASK_USER.\n"
    "\n"
